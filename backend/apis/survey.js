...
router.get("/survey/by_user", (request, response) => {
...
router.post("/survey/add", (request, response) => {
    ...
    `VALUES (${request.body.user_id}, ${request.body.age}, '${request.body.occupation}', '${request.body.income}', '${request.body.risk_tolerance}', '${request.body.investment_experience}', ` +
            `'${request.body.time_horizon}', '${request.body.investment_goal}', '${request.body.investment_knowledge}')`, (errors, results) => {
        if (errors) {
            console.log(errors);
            response.status(500).send("Internal Server Error");
        } else {
